...
[^workshop_iSEEfier-1]: [najla.abassi\@uni-mainz.de](mailto:najla.abassi@uni-mainz.de){.email}
...
```{r loadlib, echo=FALSE,eval=TRUE, message=FALSE,warning=FALSE}
library("iSEE")
library("shiny")
```
...
app <- iSEE(sce, initial = list(RowDataPlot(PanelWidth = 12L)))
```
...
app <- iSEE(sce, initial = list(ComplexHeatmapPlot(PanelWidth = 12L)))
```
...
app <- iSEE(sce, initial = list(SampleAssayPlot(PanelWidth = 12L)))
```
...
app <- iSEE(sce, initial = list(ReducedDimensionPlot(PanelWidth = 12L)))
```
